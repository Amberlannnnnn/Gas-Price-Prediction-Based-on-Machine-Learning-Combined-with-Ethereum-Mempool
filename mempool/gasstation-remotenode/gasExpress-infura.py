...
web3 = Web3(HTTPProvider('https://mainnet.infura.io/v3/7e8c9035a9fb465586ca7be563dbb2d7'))
...
#data: block_mined gas_price gasprice_gwei    
class CleanTx():
    ...
    def round_gp_10gwei(self):
        ...
        if gp >= 1 and gp < 10:
            ...
            gp = 0
        self.gp_10gwei = gp
#data blocknumber blockhash time mingp
class CleanBlock():
...
def write_to_json(gprecs, prediction_table):
    ...
    try:
        ...
        with open(filepath_gprecs, 'a') as outfile:
            json.dump(gprecs, outfile)

        with open(filepath_prediction_table, 'a') as outfile:
...
def analyze_last200blocks(block, blockdata):
    ...
    hashpower = recent_blocks.groupby('mingasprice').count()#最小gp区块的数量
    hashpower = hashpower.rename(columns={'block_number': 'count'})#原先是根据区块号-gasprice的数据，变成数量-gasprice
    hashpower['cum_blocks'] = hashpower['count'].cumsum()#累计和
...
def master_control():
    ...
    def update_dataframes(block):
        ...
        try:
            ...
            print(gprecs)
            #jsObj = json.dumps(gprecs)
            #fileobj = open('111.json','w')
            #fileobj.write(jsObj)
            #fileobj.close()
